...
function setup() {
 ...
 // trex.x=camera.position.x;
  ...
  invisibleGround = createSprite(displayWidth/2,displayHeight-10,displayWidth,125);  
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if((touches.length > 0 || keyDown("SPACE")) && trex.y  >= displayHeight-120) {
      jumpSound.play( )
      trex.velocityY = -10;
       touches = [];
    }
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(600,displayHeight-95,20,30);
